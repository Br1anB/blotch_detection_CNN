...
def loadData(datapath, numfiles):
    ...
    images = np.empty((0, 256, 256, 9))
    masks = np.empty((0, 256, 256, 3))

    for i in range(1, numfiles+1):
        #Load Respective Image and Masks
        imagepath = os.path.join(datapath, f"{i}/processed_frames.npy")
...
axs[0].imshow(XData[randomExample][:,:,3:6])
axs[0].title.set_text('Input')
axs[1].imshow(YData[randomExample])
axs[1].title.set_text('Output 1')
axs[2].imshow(XData[randomExample][:,:,:3])
axs[2].title.set_text('Frame Diff (Frame2-Frame1)')
...
# LR 0.001 WD 0.001 - Loss - 19, 0.05....
# lR 0.1?
...
for randomExample in range(0, X_test.shape[0]-1):
    ...
    fig, axs = plt.subplots(1, 3)
    axs[0].imshow(X_test[randomExample][:,:,3:6])
    axs[0].title.set_text('Input')
